...
cite_text = '"Attention is the rarest and purest form of generosity."'
cite_author = "Simone Weil"
cite_link = "https://en.wikipedia.org/wiki/Simone_Weil"
...
temp = pd.DataFrame(df2['Generosity']).reset_index()
...
temp2 = temp.sort_values(by=['Generosity'], ascending=False)[:20]
...
t1 = temp.nlargest(20, 'Generosity')[::-1]
fig2 = make_subplots(rows=1, cols=2, 
                    column_widths=[0.65, 0.35],
                    subplot_titles=['Top 20 Countries', 'All countries'])
fig2.append_trace(go.Bar(x=t1['Generosity'],
                y=t1['Country name'],
                orientation='h',
...
fig2.append_trace(go.Box(y=df['Generosity'],
...
def description_card():
    return html.Div(
        id="description_card",
        children = [dcc.Markdown(''' On the map below you see the generosity score mapped onto each country. The score ranges from -1 to 1. The results are gathered from the Gallup World Poll. Below the map you will also find a list of the Top 20 countries based on the generosity score.''')],
    style={
